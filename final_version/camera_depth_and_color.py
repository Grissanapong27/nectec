...
#config = rs.config()
#config.enable_stream(rs.stream.depth, 1280,780,rs.format.z16,30)
#pipeline.start(config)
#frames = pipeline.wait_for_frames()
#depth_frame = frames.get_depth_frame()

#depth_image = np.assanyarray(depth_frame.get_data())
#depth_colormap = cv2.applyColorMap(cv2.convertScaleAbs(depth_image, alpha =0.1),cv2.COLORMAP_JET)
	
# load the object detection network
net = detectNet(args.network, sys.argv, args.threshold)
#posenet = poseNet('resnet18-body',sys.argv.args.threshold)
        ...
        try:
            ...
            for detection in detections:
                if detection.ClassID == 1:
                    ...
                    #minDepth = 100
                    #for x_dept in range(int(detection.Left),int(detection.Right)):
                        #Depth = depth_frame.get_distance((int)(s_dept),(int)(point.y))########find code to get depth here##########
                        #if (Depth < minDepth) and (Depth > 0):
                            #self.minDepth.x = Depth
                            #depthpub.publish(self.minDepth)
