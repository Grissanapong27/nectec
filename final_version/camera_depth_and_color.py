...
class Det():
    ...
    def depthcam(self,depth_msgs):
        #convert ROS image massage to CUDA
    ...
    def camera(self,img_msgs):
        try:
            ...
            for detection in detections:
                #Make sure that object that robot detected is Human(ClassID = 1)
                if detection.ClassID == 1:
                    ...
                    print(point.x)
                    print(point.y)
                    #show Left and Right of Box 
                    print("left :",detection.Left)
                    print("Right :",detection.Right)
                    min_depth = Point()
                    #comecnt below is show depth of center of Depth image
                    #print(int(self.depth_img[360,640]))
                    #min_depth.x = int(self.depth_img[360,640])
                    min_depth.x = 300
                    x_scan = None
                    #use Depth in location of human by scan from left side of detectd object to right side
